# Read all files
read_file -format sverilog {mtr_drv.sv PWM.sv nonoverlap.sv}

# Set current design to telemetry (top module)
current_design mtr_drv
...
report_timing -path full -delay max -nworst 3 > mtr_drv_max_timing.txt
report_timing -path full -delay min -nworst 3 > mtr_drv_min_timing.txt
