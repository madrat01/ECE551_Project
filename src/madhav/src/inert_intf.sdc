# Read all files
read_file -format sverilog {inert_intf.sv SPI_mnrch.sv inertial_integrator.sv}

# Set current design to inert_intf (top module)
current_design inert_intf
...
report_area > inert_intf_area.txt

# min and max timing reports
report_timing -path full -delay max -nworst 3 > inert_intf_max_timing.txt
report_timing -path full -delay min -nworst 3 > inert_intf_min_timing.txt

# net list
write -format verilog inert_intf -output inert_intf.vg
