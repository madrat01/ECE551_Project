# Read all files
read_file -format sverilog {nonoverlap.sv}

# Set current design to telemetry (top module)
current_design nonoverlap
...
report_area > nonoverlap_area.txt

# min and max timing reports
report_timing -path full -delay max -nworst 3 > nonoverlap_max_timing.txt
report_timing -path full -delay min -nworst 3 > nonoverlap_min_timing.txt

# net list
write -format verilog nonoverlap -output nonoverlap.vg