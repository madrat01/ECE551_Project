# Read all files
read_file -format sverilog {cadence_filt.sv}

# Set current design to telemetry (top module)
current_design cadence_filt
...
report_area > cadence_filt_area.txt

# min and max timing reports
report_timing -path full -delay max -nworst 3 > cadence_filt_max_timing.txt
report_timing -path full -delay min -nworst 3 > cadence_filt_min_timing.txt

# net list
write -format verilog cadence_filt -output cadence_filt.vg